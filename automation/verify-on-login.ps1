...
if ($status -and $status.ready -and (-not $status.checks.active_month)) {
    $notification.Icon = [System.Drawing.SystemIcons]::Warning
    $notification.BalloonTipTitle = 'Automacao UPLI pausada'
    $notification.BalloonTipText = 'Marque Calendario concluido no mes atual para liberar os relatorios.'
    $notification.ShowBalloonTip(8000)
} elseif ($status -and $status.ready) {
