...
def render_status(status: dict[str, Any]) -> None:
    ...
    labels = {
        ...
        "active_month": "Calendário do mês liberado",
    }
    for key, label in labels.items():
        passed = bool(checks.get(key))
        if key == "active_month" and not passed:
            result_class = "pause"
            result_text = "PAUSADO"
        elif key == "secure_form" and not passed and not checks.get("active_month"):
            result_class = "pause"
...
def verify(allow_catchup: bool = True) -> dict[str, Any]:
    ...
    if checks["calendar"] and not checks["active_month"]:
        notes.append("Operação pausada: marque Calendário concluído no mês atual para liberar o assistente.")
    if checks["active_month"] and not checks["secure_form"]:
        issues.append("O formulário gratuito do GitHub Pages não está atualizado.")
    elif not checks["secure_form"]:
        notes.append("Os formulários ficarão disponíveis depois da publicação no GitHub Pages.")

    essential = all(checks.get(key) for key in ("config", "chrome", "internet", "calendar", "whatsapp"))
    if checks["active_month"]:
    ...
    if allow_catchup and essential and checks["active_month"] and scheduled_due(config, state):
        try:
