...
if (-not (Test-Path -LiteralPath $pythonwExe)) {
    throw 'pythonw.exe nao foi encontrado para executar a sincronizacao sem janela.'
}

$weeklyArguments = "`"$pythonScript`""
$reminderArguments = "`"$pythonScript`" --reminders"
$syncArguments = "`"$pythonScript`" --sync-responses"
$loginArguments = "-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$verifyScript`""
$weeklyAction = New-ScheduledTaskAction -Execute $pythonwExe -Argument $weeklyArguments
$reminderAction = New-ScheduledTaskAction -Execute $pythonwExe -Argument $reminderArguments
